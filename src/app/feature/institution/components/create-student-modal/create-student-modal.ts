...
export class CreateStudentModalComponent implements OnChanges, OnInit, OnDestroy {
  @Input() isOpen = false;
  ...
  submit(): void {
    ...
    this.loading = true;
    this.institutionService.createStudent({
      ...
      next: () => {
        this.loading = false;
        this.created.emit();
        this.snackbarService.success('Student created successfully.', 3000);
        this.resetForm();
      },
      error: (error: Error) => {
        this.loading = false;
